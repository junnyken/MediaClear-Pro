...
| Q-P3-08 | Khoá câu chữ nào của Phase 3 đã được owner duyệt | **100 khoá ĐÃ ĐƯỢC OWNER DUYỆT `2026-09-17`** (`D-064`) — danh sách ở `PHASE_3_CLOSURE.md` §12. Vẫn để `owner_decision_required` vì `D-064` **thêm 11 khoá mới sau khi duyệt** (`field.*`, `screen.asset_upload.pick_button`, `screen.asset_upload.no_file_chosen`); chỉ cần owner xác nhận 11 khoá này là đóng hẳn. **KHÔNG bao gồm** câu chữ xác nhận quyền — đó là `Q-11` | D-059, D-063, D-064 |
