...
| Q-11 | Có cần luật sư/BA duyệt câu chữ xác nhận quyền (vi + en) trước khi lên production không? | POLICY §3 | **`answered` — owner trả lời KHÔNG** (`2026-09-17`, `D-068`) | **Đóng bằng QUYẾT ĐỊNH của owner, KHÔNG phải do đã thẩm định pháp lý.** Owner nhận rủi ro. `Rights Statement v2` **giữ nguyên**, **không tạo v3**. Bản nháp `docs/Q11_LEGAL_WORDING_DRAFT.md` (`D-067`) **KHÔNG được áp dụng** — giữ làm tài liệu tham khảo nếu sau này owner đổi ý. **Thôi** là blocker go-live |
| Q-12 | Thư viện nào đọc được C2PA / AI provenance thật? | MCP-05 | **`answered`** (`2026-09-17`, `D-069`) | **Không dùng thư viện nào** — tự viết bộ dò `c2pa-probe.ts` (tránh native binding, và phần đắt nhất của thư viện là xác thực chữ ký thì hệ thống không cần). **Chỉ dò SỰ HIỆN DIỆN, không xác thực.** Nói `absent` ở JPEG/PNG/WebP/MP4/MOV (duyệt hết chỗ được); WebM và container lạ vẫn `unknown` theo `D-044`. Biên nhận nay khai `verified` **có cơ sở**. **Việc còn lại**: chưa đối chiếu với tệp do công cụ C2PA thật sinh ra |
...
| Q-P4-01 | Provider tracking thật nào dùng được, và benchmark ra sao? | `P4-MCP-41` | **`blocked`** | Khảo sát Bước 0 `2026-09-17`: **không có provider tracking nào** trong repo, **không** biến môi trường nào, và `PROVIDER_BENCHMARK.md` ghi **chưa chạy benchmark nào** (mọi ô `unknown`). Pipeline chạy bằng `DeterministicTrackingProvider` (tất định, **không phụ thuộc native**). Chọn provider thật phải đánh giá **rủi ro native binding** trước (OpenCV/ONNX — đúng dạng `D-039`) | **Sau `D-072`**: pipeline đã chạy trọn trong worker thật bằng bản giả tất định; provider thật **vẫn `blocked`** và **không được tự chọn**.
| Q-P4-02 | Ngưỡng `FRAME_CONFIDENCE_THRESHOLD = 0.6` có đúng không? | `P4-MCP-41` | **`unknown` — cần đo lại** | **Đây là ƯỚC LƯỢNG, không phải số đo.** Chưa có provider thật nên chưa có phân bố confidence thật. Cùng cách xử lý `Q-P3-03` (dung sai audio `0.25s`) nhưng **ngược lại**: `Q-P3-03` đã **đo** bằng 12 lượt render, con số này thì **chưa**. `FRAME_CONFIDENCE_THRESHOLD_IS_MEASURED = false` ghi đúng điều đó trong mã. Đo lại khi có provider thật. Cùng tình trạng: `MAX_MASK_SPEED_PER_SECOND = 1.5` | **Sau `D-072`**: ngưỡng **không đổi**, `FRAME_CONFIDENCE_THRESHOLD_IS_MEASURED` vẫn `false`.
| Q-P4-03 | Quy ước ID `P4-MCP-40…44` | governance | **`pending_owner_confirmation`** | Dùng `P4-` theo luật canonical `D-029` (ID là **chuỗi đầy đủ** nên `P4-MCP-40` không đụng ID nào cũ). Nhưng owner **chưa xác nhận riêng** cho Phase 4 — đúng tình trạng `Q-P3-09` trước khi được chốt. Không ID lịch sử nào bị đổi |
| Q-P4-04 | Có cần nút **hoàn tác** một lần sửa keyframe không? | `P4-MCP-42`, `D-072` | **`owner_decision_required`** | Giao diện hiện **chưa có** nút hoàn tác. Miền dữ liệu **chưa hỗ trợ**: `job_frame_corrections` là APPEND-ONLY và không có đường ghi ngược. Ghi câu hỏi thay vì **giả lập** một nút không làm đúng việc nó hứa. Nếu owner muốn, cách trung thực là **thêm một dòng correction mới** mang giá trị cũ — không xoá dòng nào |
| Q-P4-05 | Đường API `correctJobFrame` **không** tính lại frame lân cận, trong khi hàm thuần `applyCorrection` **có** — giữ điểm lệch này hay hợp nhất? | `P4-MCP-42`, `D-073` | **`owner_decision_required`** | Đo thật `2026-09-17`: mọi dòng `job_frame_corrections` ghi `reinterpolated: {}`. Hệ quả **không** âm thầm: cổng chống giật `P4-MCP-43` bắt đúng cú nhảy do sửa tay (đo được: dịch `0.43` ở fps=10, trần `0.15` ⇒ báo *"Vùng che nhảy bất thường — 2"*). Nên đây là **lựa chọn thiết kế chưa được ghi**, không phải lỗi: tính lại lân cận là **suy ra** giá trị thay người dùng, còn cách hiện tại là **hỏi lại người dùng**. Không tự hợp nhất vì hai cách cho kết quả khác nhau trên dữ liệu thật của người dùng |
...
| Q-P3-08 | Khoá câu chữ nào của Phase 3 đã được owner duyệt | **ĐÓNG — owner duyệt `2026-09-17`** (`D-065`): **100 khoá** ở `PHASE_3_CLOSURE.md` §12 **và 11 khoá thêm** của `D-064` (`field.*`, `screen.asset_upload.pick_button`, `screen.asset_upload.no_file_chosen`). Cờ `wordingOwnerApproved = true` trong `contracts/phase3-gate.ts`; thêm khoá câu chữ **mới** sau mốc này thì **phải xin duyệt lại**. **KHÔNG bao gồm** câu chữ xác nhận quyền — đó là `Q-11`, **vẫn mở** | D-059, D-063, D-064, D-065 |
